...
(function(){
  ...
  // test setting deeper nested options
  test('options deep', function(){
    var socket = new io.Socket({transportOptions:{'xhr-polling':{timeout:1}}});
    equal(socket.options.transportOptions['xhr-polling'].timeout, 1);
  });
}());