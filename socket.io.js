...
(function(){
  ...
  Flashsocket.check = function(){
    ...
    var supported = swfobject.hasFlashPlayerVersion("10.0.0");
    
    // only start downloading the .swf file when the check is done that this browser actually supports it
    // if the user disables `flashsocket` it shouldn't be loaded. This would also solve async loading issues
    // because the WebSocketJS code starts this on `load`
    supported && WebSocket.__initialize();
    return supported;
  };
  ...
  if (typeof window != 'undefined'){
    ...
    if (typeof WEB_SOCKET_SWF_LOCATION === 'undefined') WEB_SOCKET_SWF_LOCATION = '/socket.io/lib/vendor/web-socket-js/WebSocketMain.swf';
    
    // Disable AUTO_INITIALIZATION as we don't want the flash file to be loaded by default
